...
export default function PSRegisterPage() {
  ...
  const [form, setForm] = useState({ name: '', phone: '', age: '', gender: 'M', bloodGroup: '' });
  ...
  const reset = () => {
    setForm({ name: '', phone: '', age: '', gender: 'M', bloodGroup: '' });
    setSubmitted(false);
    setGeneratedId('');
    setErrors({});
  };

  const field = (key: keyof typeof form, label: string, props: Partial<React.InputHTMLAttributes<HTMLInputElement>> = {}, note?: string) => (
    <div>
        ...
        <div className="p-5 max-w-lg">
          <div className="bg-white border border-gray-100 rounded-2xl p-8 shadow-sm text-center space-y-4">
            ...
            <div className="bg-gray-50 rounded-xl px-4 py-3 text-left space-y-1">
              ...
              <p className="text-xs text-gray-500">{form.age}y · {form.gender === 'M' ? 'Male' : 'Female'}{form.bloodGroup ? ` · ${form.bloodGroup}` : ''}</p>
    ...
    <PSLayout>
      <div className="p-5 max-w-lg space-y-5">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
          {field('name', 'Full Name', { required: true, placeholder: 'Patient full name' })}
          {field('phone', 'Phone Number', { required: true, placeholder: '017XXXXXXXX' }, 'We\'ll check if this patient already exists')}
          <div className="grid grid-cols-2 gap-3">
            {field('age', 'Age', { type: 'number', required: true, min: '1', max: '120', placeholder: '42' })}
            <div>
          ...
          {field('bloodGroup', 'Blood Group', { placeholder: 'O+, A+, B-, AB+… (optional)' })}
